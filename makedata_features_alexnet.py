...
def get_image_features(net, imagedir, save=False, savefile=None):
    ...
    start_time = time()
    features = net.get_feature(imagefiles, cnn_layers, feature_num=jjjjj)
    end_time = time()
...
def get_category_averaged_features(net, imagedir, save=False, savefile=None):
    ...
    n_cat = len(catlist)
    print("Total number of Category: {}".format(n_cat))
    ...
    for i, cat in enumerate(catlist):
        ...
        print("category {:5d} of {} ({:4.2f}%)".format(i, n_cat, i * 100 / n_cat))
...
if __name__ == '__main__':
    ...
    features = get_category_averaged_features(model, './data/images/category_candidate', save=True,
                                              savefile=os.path.join(featuredir, 'feature_category_ave_candidate.pkl'))
